...
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
    ...
    <script type="text/javascript">
    	...
    	function init(){
    		doAjax('<%=request.getContextPath() + "/PaymentInf"%>',"type=consumption&id=<%=request.getParameter("id")%>",setCon);
    	}
		...
		function setCon(data, status){
                ...
                for(var i=0;i<data.length;i++){
                    a.append('<tr><td>'+data[i].id+'</td><td><a href="'+pat+'/Clerk_Member.jsp&id='+data[i].memberid+'">'+data[i].memberid+'</a></td><td><a href="'+pat+'/Clerk_Member.jsp&id='+data[i].memberid+'">'+data[i].member+'</a></td><td>'+data[i].storeid+'</td><td>'+data[i].store+'</td><td>'+data[i].productid+'</td><td>'+data[i].product+'</td><td>'+data[i].price+'</td><td>'+data[i].number+'</td><td>'+data[i].total+'</td><td>'+data[i].status+'</td><td>'+data[i].time+'</td></tr>');
                }
            }
